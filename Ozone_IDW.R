...
# air Ozone_pro for United State in the year 2000.
...
df1  <- filter(df, `Parameter Name` %in%  c("Ozonedioxide (O3)", "Ozone", "PM2.5 - Local Conditions"))
...
Ozone_1 <- SpatialPointsDataFrame(coords = xy, data = Ozone,
                                     proj4string = CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"))

# Transforming coordinate to UTM using EPSG=26916 for NAD83, UTM Zone=16N)
Ozone_pro <- spTransform(Ozone_1, CRS("+init=epsg:26916"))

# View the data property
Ozone_pro@proj4string
...
# overlay Ozoneon South Carolina boundary
plot(States_pro, border = "red")
plot(Ozone_pro, add = TRUE)
...
proj4string(grid.study_area) <- proj4string(Ozone_pro)
...
plot(Ozone_pro, add = TRUE, pch = 15, cex = 0.5, col = "blue")


neighbors = length(Ozone_pro)
beta = 2

Ozone_idw = gstat(formula = Average ~ 1, # intercept only model
                     data = Ozone_pro, 
                     nmax = neighbors, 
                     set = list(idp = beta))
...
study_area.O3 <- predict(object = Ozone_idw,
                          newdata = grid.study_area)

plot(study_area.O3, main = "IDW OzoneDioxide (02)")
plot(States_pro, add = TRUE, border = "white")
plot(Ozone_pro, add = TRUE, pch = 19, cex = 0.5, col = "green")
...
# masking
study_area_O3 <- mask(raster(study_area.O3),States_pro)



# plotting
plot(study_area_O3, main = "IDW OzoneDioxide (02) k = 2")
plot(States_pro, add = TRUE, border = "black")
plot(Ozone_pro, add = TRUE, pch = 19, cex = 0.5, col = "red")
...
class(study_area.O3)
str(study_area.O3)
spplot(study_area.O3, "var1.pred")
...
LOOCV <- krige.cv(Average~1, 
                  Ozone_pro, 
                  nfold = nrow(Ozone_pro), # Set the number of folds to the number of rows
                  set = list(idp = 2)
)
